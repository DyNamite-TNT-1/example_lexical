...
function onInput(event: InputEvent, editor: LexicalEditor): void {
  ...
  updateEditor(editor, () => {
    ...
    if (
      ...
      // ** NHANNT FIX issue korea
      // https://github.com/facebook/lexical/issues/5877

      let handled = false;
      if (data != null && $isRangeSelection(selection)) {
        const anchorNode = selection.anchor.getNode();
        const prevText = anchorNode
          .getTextContent()
          .slice(selection.focus.offset - 1, selection.focus.offset);
        const char1 = data[0];
        const char2 = prevText[0];
        if (
          editor.isComposing() &&
          selection.isCollapsed() &&
          data.length == 1 &&
